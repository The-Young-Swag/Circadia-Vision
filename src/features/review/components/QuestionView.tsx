...
export function QuestionView({ front, back, showBack, onShowBack, onHideBack }: QuestionViewProps) {
  return (
    <div className="min-h-[160px] flex flex-col">
      ...
      <div className="text-[18px] leading-relaxed font-medium mt-2">{front}</div>

      <div className="mt-6">
        {!showBack ? (
          <button className="btn-primary inline-flex items-center gap-2" onClick={onShowBack}>
          ...
          <div className="rounded-xl border border-[var(--line)] bg-[var(--surface-muted)] p-4">
            ...
            <button className="mt-3 text-xs inline-flex items-center gap-1 text-[var(--ink-faint)]" onClick={onHideBack}>
              <EyeOff size={12} /> Hide
            </button>
          </div>
        )}
