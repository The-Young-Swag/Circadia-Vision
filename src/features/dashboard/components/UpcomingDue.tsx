    ...
    <div className="card-flat p-5">
      ...
      <div className="space-y-2">
        {dueToday.slice(0, 5).map((c) => (
          <div
            key={c.id}
            className="flex items-center justify-between py-2 border-b last:border-0 border-[var(--line)]"
          >
        ...
        {dueToday.length === 0 && (
          <p className="text-sm text-[var(--ink-soft)]">
            All caught up. New cards ready: {newCount}
          </p>
        )}
      </div>
      <Link
        to="/review"
        className="btn-primary w-full mt-4 inline-flex justify-center items-center gap-2 no-underline"
      >
